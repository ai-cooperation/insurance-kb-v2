...
export const AGENT_TOOLS = [
  ...
  {name:"search_articles",description:"Agent 查找保險 KB 新聞。預設涵蓋全部可見歷史，可限日期/地區/分類。伺服器會在一次呼叫內掃完指定範圍的精簡搜尋索引，回傳全域相關度排序、總命中數與 complete=true；limit 只限制回傳前幾筆，不縮小掃描範圍。用 get_article 讀引用所指的完整保存紀錄。",inputSchema:{type:"object",properties:{...searchRange,query:{type:"string",description:"關鍵字／公司名"}},required:["query"]}},
