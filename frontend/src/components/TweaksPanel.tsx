...
interface TweaksPanelProps {
  ...
  readonly setShow: (s: boolean) => void;
  readonly tweaks: Tweaks;
  readonly setTweaks: (t: Tweaks) => void;
}

export const TweaksPanel: React.FC<TweaksPanelProps> = ({ show, setShow, tweaks, setTweaks }) => {
  if (!show) return null;

  const update = (patch: Partial<Tweaks>) => {
    const next = { ...tweaks, ...patch } as Tweaks;
    setTweaks(next);
    try {
      ...
      <div className="p-4 space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between text-[11.5px] mb-1.5">
            ...
            <span className="font-mono text-slate-500">H {tweaks.accentH}&deg;</span>
          </div>
          <input
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-1.5">
            {([
              { id: 'bordered' as const, zh: '描邊' },
              { id: 'elevated' as const, zh: '陰影' },
              { id: 'flat' as const, zh: '平面' },
            ]).map(o => (
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className={`relative w-10 h-6 rounded-full transition ${tweaks.dark ? 'bg-accent' : 'bg-slate-300 dark:bg-slate-700'}`}
          >
